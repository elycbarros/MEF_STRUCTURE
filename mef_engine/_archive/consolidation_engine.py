...
def _primary_settlement(layer: ConsolidationLayer, delta_sigma: float) -> float:
    """
    ...
    if svf <= sp:
        ...
        # Parcialmente OC → NC
        settlement_m = (Cs / (1.0 + e0)) * H * np.log10(sp / sv0) + (Cc / (1.0 + e0)) * H * np.log10(svf / sp)
    else:
...
def _secondary_settlement(layer: ConsolidationLayer, t_years: float, t_primary_years: float) -> float:
    """
...
def run_consolidation_analysis(
    ...
    ratio_secondary = (
        (settlement_curve[-1]['settlement_secondary_mm'] / max(final_total, 1e-9)) if settlement_curve else 0.0
    )
