...
class ColumnDetailer:
    ...
    def calculate_stirrups(b_m: float, h_m: float, phi_long_mm: float, Nd_kN: float) -> dict:
        ...
        if phi_est_mm < 6.3:
            phi_est_mm = 6.3  # Mínimo prático em pilares
        ...
        return {'phi_mm': phi_est_mm, 'spacing_cm': s_final, 'label': f'Φ {phi_est_mm:.1f} c/{s_final}'}
    ...
    def calculate_executive_geometry(b_m: float, h_m: float, longit: dict) -> dict:
        ...
        return {'rebar_coords': [{'x': round(c[0], 3), 'y': round(c[1], 3)} for c in coords], 'cover_m': cover}
    ...
    def generate_detailing_summary(solve_result: dict) -> dict:
        ...
        return {
            ...
            'notes': ['Concreto C' + str(solve_result.get('fck_MPa', 30)), 'Aço CA-50', 'Cobrimento 3.0 cm'],
        }


if __name__ == '__main__':
    # Teste rápido
    mock_res = {'As_final_cm2': 18.5, 'section': '40x40', 'fck_MPa': 30, 'Nd_kN': 2500}
