...
def calculate_rigid_soil_pressure(cfg: AnalyticalConfig) -> dict:
    ...
    corners = [(0.0, 0.0), (cfg.Lx, 0.0), (cfg.Lx, cfg.Ly), (0.0, cfg.Ly)]
...
def calculate_analytical_moments(cfg: AnalyticalConfig, rigid: dict) -> dict:
    ...
    return {'m_ref_analitico_kNm_m': float(m_ref), 'vao_referencia_m': float(l_ref)}


def calculate_analytical_comparison(
    mef_summary: dict, cfg: AnalyticalConfig, punching_mef_df: pd.DataFrame | None = None
) -> dict:
    ...
    comp = {
        ...
        'mef': {'q_med_kPa': q_mef_med, 'q_max_kPa': q_mef_max, 'm_max_kNm_m': m_mef_max},
        'divergence_metrics': {
            'q_med_diff_pct': (q_mef_med / q_an_med - 1.0) if q_an_med > 0 else 0.0,
            'q_max_diff_pct': (q_mef_max / q_an_max - 1.0) if q_an_max > 0 else 0.0,
            'm_diff_pct': (m_mef_max / moments_an['m_ref_analitico_kNm_m'] - 1.0)
            if moments_an['m_ref_analitico_kNm_m'] > 0
            else 0.0,
        },
        'momento_max_mef_kNm_m': m_mef_max,
        'momento_ref_analitico_kNm_m': moments_an['m_ref_analitico_kNm_m'],
        'ratio_momento': m_mef_max / moments_an['m_ref_analitico_kNm_m']
        if moments_an['m_ref_analitico_kNm_m'] > 0
        else 1.0,
        'punching_comparison': [],
    }
