...
def run_unified_analysis(frame_config: LajeLabV2Config, radier_config: LabConfig, wind_config: WindConfig = None):
    ...
    new_pillars_for_radier.append(
        PillarSupport(id='ESCADA_A1', x=2.0, y=2.0, p_kN=stair['total_reaction_kN'], bx=0.4, by=1.2)
    )
    new_pillars_for_radier.append(
        PillarSupport(id='TANK_B1', x=20.0, y=20.0, p_kN=tank['total_load_kN'], bx=1.0, by=1.0)
    )
    ...
    wind_res = wind_engine.generate_force_profile(
        height=total_height, width=radier_config.Lx, depth=radier_config.Ly, step=floor_height
    )
