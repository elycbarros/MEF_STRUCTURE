...
def _interaction_factor_poulos(
    s: float, d: float, L: float, Es: float, Ep: float, nu: float, pile_type: str = 'friction'
) -> float:
    """
...
def compute_interaction_matrix(piles: list, config: PileGroupConfig) -> np.ndarray:
    """
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            a = _interaction_factor_poulos(
                s, d_avg, L_avg, config.Es_kPa, config.Ep_kPa, config.nu_soil, config.pile_type
            )
            alpha[i, j] = a
            alpha[j, i] = a
...
def compute_group_efficiency(piles: list, config: PileGroupConfig) -> dict:
    """
