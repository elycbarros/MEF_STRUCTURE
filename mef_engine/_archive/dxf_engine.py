...
class RadierDXFEngine:
    ...
    def draw_mesh(self, layer: str, spacing_m: float, label: str):
        ...
        self.msp.add_text(label, dxfattribs={'layer': 'TEXTO', 'height': 0.25}).set_placement(
            (self.Lx / 2, self.Ly / 2)
        )
    ...
    def draw_columns(self, column_loads: np.ndarray):
        ...
        for col in column_loads:
            ...
            p = [(x - 0.3, y - 0.3), (x + 0.3, y - 0.3), (x + 0.3, y + 0.3), (x - 0.3, y + 0.3), (x - 0.3, y - 0.3)]
            self.msp.add_lwpolyline(p, dxfattribs={'layer': 'PILOTIS'})
            self.msp.add_text('P', dxfattribs={'layer': 'TEXTO', 'height': 0.15}).set_placement((x + 0.4, y + 0.4))
    ...
    def draw_punching_reinforcement(self, punching_results: list[dict], d_m: float):
        ...
        for res in punching_results:
            if res.get('status') == 'REFORÇO':
                ...
                self.msp.add_text('REFORÇO PUNÇÃO', dxfattribs={'layer': 'TEXTO', 'height': 0.10}).set_placement(
                    (x - bx / 2 - offset, y + by / 2 + offset + 0.1)
                )
    ...
    def draw_title_block(self, project_name: str, client: str, author: str):
        ...
        self.msp.add_lwpolyline(
            ...
            dxfattribs={'layer': 'GEOMETRIA', 'color': 7},
        )
        ...
        # Textos
        self.msp.add_text('PROJETO ESTRUTURAL', dxfattribs={'layer': 'TEXTO', 'height': 0.25}).set_placement(
            (x0 + 0.2, y0 + h - 0.4)
        )
        self.msp.add_text(project_name.upper(), dxfattribs={'layer': 'TEXTO', 'height': 0.4}).set_placement(
            (x0 + 0.2, y0 + h - 0.9)
        )

        self.msp.add_text(f'CLIENTE: {client}', dxfattribs={'layer': 'TEXTO', 'height': 0.2}).set_placement(
            (x0 + 0.2, y0 + h - 1.6)
        )
        self.msp.add_text(f'RESPONSÁVEL: {author}', dxfattribs={'layer': 'TEXTO', 'height': 0.2}).set_placement(
            (x0 + 0.2, y0 + h - 2.1)
        )

        self.msp.add_text('ESCALA: 1:50  |  DATA: 2026', dxfattribs={'layer': 'TEXTO', 'height': 0.15}).set_placement(
            (x0 + 0.2, y0 + 0.3)
        )
        self.msp.add_text(
            'MEF STRUCTURAL - V5 PLENO', dxfattribs={'layer': 'TEXTO', 'height': 0.15, 'color': 1}
        ).set_placement((x0 + w - 3.5, y0 + 0.3))
    ...
    def draw_bar_list(self, steel_data: list[dict]):
        ...
        """
        x0, y0 = self.Lx + 1, 5
        self.msp.add_text('TABELA DE AÇO (RESUMO)', dxfattribs={'layer': 'TEXTO', 'height': 0.35}).set_placement(
            (x0, y0 + 4.5)
        )
        ...
        for idx, item in enumerate(steel_data):
            ...
            self.msp.add_text(f'{length:.2f}', dxfattribs={'layer': 'TEXTO', 'height': 0.18}).set_placement(
                (x0 + col_w[0] + col_w[1], y)
            )
            self.msp.add_text(f'{weight:.2f}', dxfattribs={'layer': 'TEXTO', 'height': 0.18}).set_placement(
                (x0 + col_w[0] + col_w[1] + col_w[2], y)
            )

        self.msp.add_line((x0, y0 - 0.2), (x0 + sum(col_w), y0 - 0.2), dxfattribs={'layer': 'GEOMETRIA'})
        self.msp.add_text(
            f'PESO TOTAL: {total_weight:.2f} kg', dxfattribs={'layer': 'TEXTO', 'height': 0.25}
        ).set_placement((x0, y0 - 0.7))
...
class SpecialElementsDXFEngine:
    ...
    def draw_retaining_wall(self, h_wall: float, b_base: float, thick_wall: float):
        ...
        p = [
            ...
            (b_base, 0.4),  # Base
            (thick_wall + 0.2, 0.4),
            (thick_wall, h_wall),  # Parede
            (0, h_wall),
            (0, 0),
        ]
        self.msp.add_lwpolyline(p, dxfattribs={'layer': 'GEOMETRIA'})
        self.msp.add_text('CORTE MURO DE ARRIMO', dxfattribs={'layer': 'TEXTO', 'height': 0.2}).set_placement(
            (0, h_wall + 0.5)
        )
    ...
    def draw_stair_section(self, l_horiz: float, h_vert: float, steps: int):
        ...
        self.msp.add_text('DETALHE ESCADA', dxfattribs={'layer': 'TEXTO', 'height': 0.2}).set_placement(
            (0, h_vert + 0.5)
        )
