...
class BeamDetailer:
    ...
    def calculate_skin_reinforcement(h_cm: float, b_cm: float) -> Dict:
        ...
        return {'needed': True, 'As_skin_cm2_face': round(As_skin, 2), 'suggested': 'Φ 5.0 c/20 (Face)'}
    ...
    @staticmethod
    def calculate_executive_geometry(
        L_beam_m: float, b_m: float, h_m: float, det_inf: dict, det_sup: dict, al_cm: float
    ) -> dict:
        """
    ...
    @classmethod
    def generate_detailing_summary(
        cls,
        design_res: dict,
        b_m: float,
        h_m: float,
        ...
        return {
            ...
            'torsion': {'Asl_cm2': Asl_torsion, 'status': 'REQUERIDA' if Asl_torsion > 0 else 'NÃO REQUERIDA'},
...
class ServiceabilityEngine:
    ...
    def calculate_crack_width(
        fck: float, b_cm: float, h_cm: float, As_efet: float, M_service: float, phi_mm: float
    ) -> Dict:
        """
    ...
    def calculate_effective_inertia(
        E_mpa: float, fctm_mpa: float, I_gross: float, I_cracked: float, M_cracking: float, M_service: float
    ) -> float:
        """
