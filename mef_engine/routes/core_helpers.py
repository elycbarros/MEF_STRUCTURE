...
def build_field_risk_summary(input: ConfigInput) -> dict[str, Any]:
    ...
    if is_laje:
        return {
            "status": "green",
            "selected": [],
            "recommendations": ["Análise de laje suspensa: riscos geotécnicos de subleito não se aplicam diretamente ao elemento."],
...
def build_winkler_consistency(input: ConfigInput, deterministic: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_laje:
        return {
            "available": False,
            "note": "Análise de Winkler não aplicável a lajes suspensas."
        }
...
def build_foundation_recommendation(input: ConfigInput, memorial: dict[str, Any], deterministic: dict[str, Any], field_risk_summary: dict[str, Any], winkler: dict[str, Any]) -> dict[str, Any]:
    ...
    if not is_laje:
        if geotech.get("atende_pressao_max_modelo") is False:
            add_trigger("soil_pressure_exceeded", "bloqueio", "red", "Capacidade de carga do solo excedida: a pressão de contato no modelo supera a resistência admissível do terreno, gerando risco de ruptura ou recalques excessivos.", f"qmax/sigma_adm = {pressure_ratio:.2f}")
        elif pressure_ratio >= profile["pressure_restriction_ratio"]:
            add_trigger("soil_pressure_restriction", "restricao", "yellow", "Carga no solo em nível crítico: a pressão está muito próxima do limite de segurança. Recomenda-se prova de carga ou revisão do kv.", f"qmax/sigma_adm = {pressure_ratio:.2f}")
        elif pressure_ratio >= profile["pressure_alert_ratio"]:
            add_trigger("soil_pressure_high", "alerta", "yellow", "Solicitação do solo elevada: a margem de segurança geotécnica está reduzindo. Monitorar recalques.", f"qmax/sigma_adm = {pressure_ratio:.2f}")

    if punching.get("atende") is False:
        add_trigger("punching_failure", "bloqueio", "red", "Risco de punção (furo): a concentração de carga no pilar supera a resistência do concreto à tração diagonal. Requer engrossamento da laje ou armadura de punção.", f"eta_puncao = {punching_ratio:.2f}")
    elif punching_ratio >= profile["punching_restriction_ratio"]:
        add_trigger("punching_restriction", "restricao", "yellow", "Punção em limite crítico: a reserva de resistência à tração diagonal é mínima. Estudar reforço local ou capitéis.", f"eta_puncao = {punching_ratio:.2f}")
    elif punching_ratio >= profile["punching_alert_ratio"]:
        add_trigger("punching_high", "alerta", "yellow", "Atenção ao efeito de punção: solicitação alta em torno dos pilares. Pode exigir redução do espaçamento da armadura superior.", f"eta_puncao = {punching_ratio:.2f}")

    if not is_laje:
        if recalc.get("atende_global") is False:
            add_trigger("settlement_failure", "bloqueio", "red", "Recalques fora dos limites: os deslocamentos verticais ou distorções angulares superam as tolerâncias da superestrutura (NBR 6118).", f"wmax = {_as_float(service.get('w_max_mm')):.2f} mm")
        else:
            service_ratios = [
                ("settlement_total_high", "Recalque total elevado: deslocamento vertical absoluto próximo ao limite de serviço.", total_settlement_ratio),
                ("settlement_differential_high", "Recalque diferencial elevado: diferença de nível entre apoios pode gerar esforços secundários na estrutura.", differential_settlement_ratio),
                ("angular_distortion_high", "Distorção angular crítica: a inclinação relativa entre pilares pode causar fissuração em alvenarias e acabamentos.", angular_distortion_ratio),
            ]
            for code, message, ratio in service_ratios:
                ...
                if ratio >= restriction_threshold:
                    add_trigger(code.replace("_high", "_restriction"), "restricao", "yellow", message.replace("elevado:", "em limite de restrição:"), f"uso/limite = {ratio:.2f}")
    ...
    if service.get("wk_x_ok") is False or service.get("wk_y_ok") is False:
        add_trigger("cracking_failure", "restricao", "yellow", "Fissuração excessiva (Estado Limite de Serviço): abertura de frestas acima do permitido para a classe de agressividade ambiental.", f"wk_x_ok={service.get('wk_x_ok')} | wk_y_ok={service.get('wk_y_ok')}")
    elif wk_ratio is not None and wk_ratio >= profile["cracking_restriction_ratio"]:
        add_trigger("cracking_restriction", "restricao", "yellow", "Fissuração em limite de atenção: risco de durabilidade por exposição da armadura a agentes agressivos.", f"wk/limite = {wk_ratio:.2f}")
    elif wk_ratio is not None and wk_ratio >= profile["cracking_alert_ratio"]:
        add_trigger("cracking_high", "alerta", "yellow", "Controle de fissuração: abertura de fissuras próxima ao limite normativo. Recomendado reduzir bitolas e espaçamentos.", f"wk/limite = {wk_ratio:.2f}")
    ...
    if is_laje:
        ...
        if benchmark.get("blocks_professional_use", not benchmark.get("all_passed", False)):
            add_trigger("benchmark_failure", "bloqueio", "red", "Auditoria de precisão: divergência detectada frente ao benchmark de segurança calibrado para este motor. Uso bloqueado para prevenir erros estruturais.", f"suite={benchmark.get('suite_name', 'n/d')}")
        ...
        if math.isfinite(winkler_ratio) and winkler_ratio < profile["winkler_min_ratio"]:
            add_trigger("winkler_inconsistency", "bloqueio", "red", "Inconsistência de Winkler: o recalque do modelo é muito inferior à média teórica (q/kv). Verifique rigidez e vinculações.", f"wmax/wmedio = {winkler_ratio:.2f}")
        elif not math.isfinite(winkler_ratio) and winkler.get("pass") is False:
            add_trigger("winkler_inconsistency", "bloqueio", "red", "Inconsistência de Winkler: falha na validação de convergência do modelo elástico linear.", f"wmax/wmedio = {winkler.get('wmax_over_expected_mean')}")
        elif math.isfinite(winkler_ratio) and winkler_ratio < profile["winkler_alert_ratio"]:
            add_trigger("winkler_low_margin", "alerta", "yellow", "Comportamento de Winkler atípico: recalque próximo da média teórica. Avaliar sensibilidade à variação de kv nas bordas.", f"wmax/wmedio = {winkler_ratio:.2f}")
    ...
    for item in triggers:
        ...
        if level == "bloqueio":
            ...
            priority_actions.append("Refinamento de Piso: detalhar juntas de retração e verificar fadiga sob cargas móveis cíclicas.")
...
def build_thermal_checklist(input: ConfigInput) -> dict[str, Any]:
    ...
    return {
        ...
        ) if applicable else ("Sistemas de laje suspensa ou espessura abaixo do limiar (0.80 m): Checklist de concreto massa não aplicável."),
    }
