...
def calculate_branson_inertia(M_max: float, fck: float, h: float, d: float, As_m2_m: float, E_c: float, E_s: float = 210e9) -> float:
    """
...
def redistribute_moments(
    ...
    if not config.enabled:
        return {
            'mx': mx.copy(), 'my': my.copy(), 'mxy': mxy.copy(),
    ...
    if peak_mask.sum() == 0 or field_mask.sum() == 0:
        return {
            'mx': mx_red, 'my': my_red, 'mxy': mxy.copy(),
            'redistribution_applied': False,
            'peak_reduction_pct': 0.0,
            'n_peaks': 0,
...
def calculate_long_term_stiffness_reduction(
    M_service: float, 
    params: ConcreteNonlinearParams
) -> float:
    """
...
class NonlinearAuditTrail:
    ...
    def log_iteration(self, it_num: int, max_diff: float, n_cracked: int):
        self.iterations.append({
            'iteration': it_num,
            'max_stiffness_diff': float(max_diff),
            'n_elements_cracked': int(n_cracked)
        })
...
class ConcreteNonlinearEngine:
    ...
    def run_iterative_analysis(
        ...
        if long_term:
            ...
            if verbose: print(f"⏳ Aplicando Coeficiente de Fluência (Phi=2.0) para análise de longo prazo.")
        else:
            E_c_calc = E_c
    ...
    def compute_stiffness_reduction(moments_nm_m: np.ndarray, model: any) -> np.ndarray:
        ...
        if hasattr(model, 'fck'): fck = model.fck
        elif hasattr(model, 'material') and hasattr(model.material, 'fck'): fck = model.material.fck
