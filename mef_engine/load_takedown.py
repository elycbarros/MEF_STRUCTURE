...
class LoadTakedownEngine:
    @staticmethod
    def run_vertical_takedown(n_floors: int, area_influence_m2: float, 
                             q_dead_kPa: float = 5.0, q_live_kPa: float = 2.0) -> pd.DataFrame:
        """
        ...
        for i in range(n_floors, 0, -1):
            ...
            if (n_floors - i) > 3: reduction_factor = 0.8
            if (n_floors - i) > 6: reduction_factor = 0.6
