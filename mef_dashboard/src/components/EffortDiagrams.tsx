...
export default function EffortDiagrams({ memberId, data, type, unit, title, variant = "dark", comparisonData, comparisonLabel }: EffortDiagramsProps) {
  ...
  return (
    <div className={cn(
      ...
      isLight ? "border-slate-200 bg-white" : "border-white/10 bg-white/5 backdrop-blur-xl",
    )}>
      <div className="flex justify-between items-center mb-4">
        <div>
          <h3 className={cn("text-sm font-bold uppercase tracking-widest", isLight ? "text-slate-950" : "text-white/90")}>{resolvedTitle}</h3>
          <p className={cn("mt-1 text-[10px] font-semibold", isLight ? "text-slate-500" : "text-white/40")}>Arraste o cursor para ler uma seção.</p>
        </div>
        <span className={cn("text-[10px] font-mono", isLight ? "text-slate-500" : "text-white/40")}>Membro #{memberId} ({unit})</span>
      </div>

      {selected && (
        <div className={cn(
          "mb-3 grid grid-cols-2 gap-2 rounded-xl border px-3 py-2 text-xs",
          isLight ? "border-slate-200 bg-slate-50 text-slate-700" : "border-white/10 bg-black/20 text-white/75",
        )}>
          <div>
      ...
      <div className="mt-4 flex flex-wrap gap-4">
        <div className="flex items-center gap-2">
           ...
           <span className={cn("text-[10px]", isLight ? "text-slate-600" : "text-white/60")}>Esforço solicitante</span>
        </div>
        <div className="flex items-center gap-2">
           <div className={cn("w-2 h-0.5", isLight ? "bg-slate-300" : "bg-white/20")}></div>
           <span className={cn("text-[10px]", isLight ? "text-slate-600" : "text-white/60")}>Linha de eixo</span>
        </div>
        {comparisonLabel && diagram.compLinePath && (
          <div className="flex items-center gap-2">
            ...
            <span className={cn("text-[10px]", isLight ? "text-slate-600" : "text-white/60")}>{comparisonLabel}</span>
          </div>
        )}
        {maxPoint && (
          <div className="flex items-center gap-2">
        ...
        <div className={cn("ml-auto text-[10px] font-mono", isLight ? "text-slate-400" : "text-white/35")}>
          {data.length} seções
        </div>
      </div>
    </div>
